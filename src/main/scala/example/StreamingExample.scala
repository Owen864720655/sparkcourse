...
object StreamingExample {
  ...
  def main(args: Array[String]) {
    ...
    def usage(): Unit = System.err.println("Usage: HdfsWordCount --input inputdir --checkpoint checkpointdir --stopwords stopfile")
    def nextOption(map: Map[Symbol, Any], list: List[String]): Map[Symbol, Any] = {
      ...
      list match {
        ...
        case "--checkpoint" :: value :: tail =>
          nextOption(map ++ Map('checkpoint -> value), tail)
    ...
    ssc.checkpoint(options('checkpoint).toString())
    ssc.start()
